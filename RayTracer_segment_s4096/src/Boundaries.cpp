 ...
 * Adapted from: http://gamedev.stackexchange.com/a/18459
 */
bool Boundaries::intersect(const Ray& ray, const Vector &maxv, const Vector &minv) {
  return (  (minv.x <= max.x) && (minv.y <= max.y) && (minv.z <= max.z)
         && (maxv.x >= min.x) && (maxv.y >= min.y) && (maxv.z >= min.z)   );
}
