...
Intersection RayTracer::getClosestIntersection(const Ray& ray) {
   ...
   double segmentLength = 4096.0;
   Intersection inte;
   Vector origin = ray.origin;
   Vector dest = origin + ray.direction * segmentLength;
   Vector maxv, minv;

   minv.x = min(origin.x, dest.x);
   minv.y = min(origin.y, dest.y);
   minv.z = min(origin.z, dest.z);
   maxv.x = max(origin.x, dest.x);
   maxv.y = max(origin.y, dest.y);
   maxv.z = max(origin.z, dest.z);
   
   inte = bsp->getClosestIntersection(ray, maxv, minv);
   origin = (origin + dest) / 2.0;
   segmentLength /= 2.0;
   while(segmentLength < 20000.0 && !inte.didIntersect){
      segmentLength *= 2;
      origin = dest;
      dest = origin + ray.direction * segmentLength;
   ...
   if (!inte.didIntersect){ //no intersection with length 50000
      inte = bsp->getClosestIntersection(ray);
   }
   return inte;
}
